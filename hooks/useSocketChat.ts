    ...
    const sendQueuedMessages = () => {
			...
			queuedMessages.forEach((queuedMsg) => {
				const messageData = {
					...
					type: queuedMsg.type,
					isEncrypted: queuedMsg.isEncrypted,
					encryptionKey: queuedMsg.encryptionKey,
				};

				console.log("📤 Sending queued message:", messageData);
				socket.emit("message", messageData);
				console.log("🔄 Updating queued message status to sent for:", {
					recipientId: queuedMsg.recipientId,
			...
			const handlePendingMessages = (data: {
				...
				data.messages.forEach((msg) => {
					const socketMessage = {
						...
						type: msg.type,
						id: msg.id,
						isEncrypted: !!msg.isEncrypted, // always include, default to false if missing
						encryptionKey: msg.encryptionKey, // include if present
					};
					addSocketMessage(socketMessage);
		...
		const sendMessage = (
			...
			if (isEncrypted) messageData.isEncrypted = true;
			if (encryptionKey) messageData.encryptionKey = encryptionKey;
			...
			if (socket && isConnected) {
				...
				console.log("🔄 Updating message status to sent for:", {
					recipientId,
					message,
				});
