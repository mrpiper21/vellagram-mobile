...
export interface ChatState {
	...
	createConversation: (
		conversation: Omit<
			Conversation,
			"id" | "createdAt" | "updatedAt" | "lastMessageTime" | "unreadCount"
		>
	) => void;
	updateConversation: (
		recipientId: string,
	...
	addToQueue: (
		message: Omit<QueuedMessage, "localId" | "timestamp" | "status">
	) => void;
	...
	persist(
		(set, get) => ({
				...
				set((state) => {
					...
					const updatedConversation: Conversation = conversation
						? {
								...
								unreadCount: conversation.participants.includes(
									message.senderId
								)
						...
						: {
								id: conversationId,
								participants: message.recipientId
			...
			deleteConversation: (recipientId) => {
				set((state) => {
					const {
						[recipientId]: deletedConversation,
						...remainingConversations
					} = state.conversations;
					const { [recipientId]: deletedMessages, ...remainingMessages } =
						state.messages;
			...
			resetChatStore: () => {
				...
				console.log(
					"🔄 Chat store reset - all conversations and messages cleared"
				);
			},
			...
			drainQueue: (sentLocalIds: string[]) => {
				set((state) => ({
					queuedMessages: state.queuedMessages.filter(
						(msg) => !sentLocalIds.includes(msg.localId)
					),
				}));
			},
