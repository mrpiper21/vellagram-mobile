...
const ContactsScreen = () => {
	...
	const filteredContacts = useMemo(() => {
		...
		const registeredContacts = contactsToFilter.filter(
			(contact) => contact.isRegistered
		);
		const unregisteredContacts = contactsToFilter.filter(
			(contact) => !contact.isRegistered
		);
	...
	const keyExtractor = useCallback(
		(item: ContactWithRegistration, index: number) => `${item.id}-${index}`,
		[]
	);
